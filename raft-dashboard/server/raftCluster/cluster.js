...
const METRICS = await initMetrics({
  ...
  timeoutLowMs: 6000, timeoutHighMs: 10000,
});
...
let currentTerm = 0;
let pendingFTCandidate = null;
let electionPrimed = false;
let electionRoundStartTs = 0;

const ELECTION_ROUND_MS = 3000;
...
const ELECTION_TIMEOUT_JITTER = 4000;

const randomElectionTimeout = () => ELECTION_TIMEOUT_MIN + Math.floor(Math.random() * ELECTION_TIMEOUT_JITTER);
const nextElectionDeadline = () => now() + randomElectionTimeout() + Math.floor(Math.random() * 300);
...
const setPaused = (p) => {
  if (p && !paused) { paused = true; pausedAt = Date.now(); return; }
  if (!p && paused) {
    const delta = Date.now() - pausedAt;
    paused = false; pausedAt = 0;
    inflight.forEach(m => { m.arriveAt += delta; m.sendAt += delta; });
    ids().forEach(id => {
      const r = runtime?.[id]; if (!r) return;
      if (Number.isFinite(r.heartbeatDueMs)) r.heartbeatDueMs += delta;
      if (Number.isFinite(r.electionDeadlineMs)) r.electionDeadlineMs += delta;
    });
  }
};
...
const becomeLeader = (id) => {
  ids().forEach((pid) => {
    ...
    if (pid === id) {
      r.role = "leader";
      r.votedFor = null;
      r.heartbeatDueMs = now() + Math.min(400, HEARTBEAT_MS);
    } else {
...
const sendHeartbeats = (leaderId) => {
  ...
  ids().forEach((fid) => {
    ...
    if (fr && nextIndex < committedLogs.length) {
      oneEntry = committedLogs[nextIndex];
    } else if (fr && nextIndex < lr.log.length) {
      oneEntry = lr.log[nextIndex];
    }
...
const scheduleRequestVotes = (candidateId) => {
  ...
  if (!c) return;
  const prevTerm = c.term || 0;
  const nextTerm = prevTerm + 1;
  if (c._rvSentTerm === nextTerm) return;
  c.role = "candidate";
  c._rvSentTerm = c.term;
  ...
  ids().forEach((pid) => {
    ...
    scheduleBall({
      ...
      toId: pid,
      travelMs: BALL_TRAVEL_MS,
      payload: { term: c.term, candidateId, lastLogIndex: cLastIdx, lastLogTerm: cLastTerm },
      onDeliver: ({ payload }) => {
        if (crashedNodes.has(pid)) return;
...
const runElectionStep = () => {
  ...
  if (electionCandidates.length === 0) {
    ...
    if (!prevLeaderSnapshot) {
      ...
      if (prevId) {
        prevLeaderSnapshot = { id: prevId, term: runtime[prevId].term || 1, log: (runtime[prevId].log || []).slice() };
      }
    }
    electionCandidates = pickCandidates();
    voteTally = {};
    electionCandidates.forEach(cid => scheduleRequestVotes(cid));
    electionPrimed = false;
    electionRoundStartTs = now();
  } else {
    electionPrimed = false;
  }
};
...
export const advanceStep = () => {
  ...
  if (mode === MODE_DYNAMIC && !getLeaderId()) {
    ...
    if (pendingFTCandidate) {
      electionCandidates = [pendingFTCandidate];
      electionPrimed = true;
    } else {
      electionCandidates = [];
      voteTally = {};
  ...
  if (mode === MODE_ELECTION) {
    ...
    if (winnerEntry) {
      ...
      const roundElapsed = now() - electionRoundStartTs;
      const rvInFlight = inflight.some(m =>
        m.type === "requestVote" || m.type === "voteGiven"
      );
      if (roundElapsed >= ELECTION_ROUND_MS && !rvInFlight) {
        const tried = new Set(electionCandidates);
        let nextPool = pickCandidates().filter(id => !tried.has(id));
        if (nextPool.length === 0) {
          nextPool = aliveIds().filter(id =>
            runtime[id]?.role !== "leader" && hasCommittedPrefix(runtime[id])
          );
        }
        if (nextPool.length === 0) {
          nextPool = aliveIds().filter(id => runtime[id]?.role !== "leader");
        }
        voteTally = {};
        electionCandidates = nextPool.slice(0, 2);
        electionCandidates.forEach(cid => scheduleRequestVotes(cid));
        electionRoundStartTs = now();
        pendingFTCandidate = null;
      }
